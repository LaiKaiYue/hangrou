...
- 📊 銷售報表：
  ...
  - **訂單數區塊**：右下角顯示「>> 詳細資料」提示，懸停時變色，點擊查看訂單明細
  - 分類統計、時段分析
  - **歷史查詢**：
    - 日報表：日期選擇器、今日/昨日快速切換
    - 週報表：週選擇器、本週/上週快速切換
    - 月報表：月份選擇器、本月/上月快速切換
  - **訂單明細列表**：
    - **折疊功能**：預設收起，點擊訂單標題展開/收起明細
    - **批次操作**：搜尋框下方提供「全部展開」和「全部收起」小連結
    - **即時響應**：移除動畫效果，點擊立即展開/收起，無卡頓
    - **箭頭切換**：▶ → ▼ 即時切換（無動畫延遲）
    - **遞減排序**：依訂單編號遞減排序（最新訂單在最上方）
    - **懸停效果**：滑鼠懸停背景變色，點擊有壓下效果
    - **折扣顯示優化**：
      - **訂單標題右邊**：顯示完整折扣（品項折扣 + 自動折扣 + 手動折扣）
      - **小計下方**：顯示折扣（自動折扣 + 手動折扣，不含品項折扣）
      - **品項明細**：獨立顯示品項折扣欄位（💰 品項折扣：-$XX）
      - **計算邏輯**：小計（品項折扣後）- 自動折扣 - 手動折扣 = 總計
  - **每日結帳條**：
    - 完整銷售明細列表
    - **匯出結帳明細**（支援日期區間查詢匯出）
      - 自動讀取日期選擇器
...
- 🔧 **狀態管理架構重構**（為 Vue 3 轉換做準備）：
  - **統一 appState 物件**：整合 20+ 全域變數，分類管理
    ...
    - `spec`：規格選擇（辣度、蔵蒜醬、客製化、飲料規格、**當前品項折扣**）
...
- [ ] 連續點擊不同快速金額按鈕，無卡頓
- [ ] 測試規格選擇按鈕觸控優化：
  - [ ] 快速連續點擊辣度按鈕（不辣→微辣→中辣→大辣），無延遲
  - [ ] 快速連續點擊胡椒按鈕（正常→胡椒多→胡椒少→去胡椒），無卡頓
  - [ ] 快速連續點擊醬按鈕（正常→醬多→醬少→去醬），無卡頓
  - [ ] 飲料規格快速切換（冰/熱、冰塊、甜度），無延遲
  - [ ] 統一規格快速切換（辣度、胡椒、醬），按鈕立即響應
  - [ ] 驗證按鈕按下有視覺回饋（縮小效果）
  - [ ] 測試震動回饋（如果 iPad 支援）
  - [ ] 驗證滾動時不會誤觸發按鈕
- [ ] 驗證沒有雙擊放大或雙指縮放
...
- [ ] 測試訂單明細折疊功能
  ...
  - [ ] 驗證點擊訂單標題可展開/收起
  - [ ] 驗證箭頭從 ▶ 切換成 ▼（即時無延遲）
  - [ ] 驗證展開/收起無卡頓（移除動畫，即時響應）
  - [ ] 驗證訂單按編號遞減排序（最新在最上方）
  - [ ] 驗證懸停時背景變色效果
  - [ ] 驗證點擊有壓下效果
  - [ ] 測試批次操作功能
    - [ ] 驗證「全部展開」和「全部收起」連結顯示在搜尋框下方
    - [ ] 驗證連結為小文字樣式（非大按鈕）
    - [ ] 驗證點擊「全部展開」所有訂單同時展開
    - [ ] 驗證點擊「全部收起」所有訂單同時收起
    - [ ] 驗證批次操作無卡頓流暢
...
- ✅ 觸控優化（零延遲響應、觸覺回饋、防止干擾）
  - **主畫面優化**：菜單項目使用 touchstart 事件（消除 300ms 延遲）
  - **規格選擇優化**：所有規格按鈕（辣度、胡椒、醬、飲料規格、統一規格）使用 touchstart
  - **按鈕視覺回饋**：.pressed class（scale(0.95) 縮小效果）
  - **觸覺回饋**：震動 10ms（Vibration API）
  - **智能判斷**：區分點擊和滾動（移動 > 10px 視為滾動）
  - **快速連續點擊**：觸控時間 < 800ms 且未移動才觸發
  - **統一規格優化**：使用 requestAnimationFrame 延遲執行購物車更新
- ✅ 測試資料生成（過去7天）
- ✅ **折扣系統優化**（購物車顯示優化、結帳小計折扣後、訂單明細優化）
- ✅ **購物車顯示**（品項總額右下角、單價×數量、品項折扣獨立欄位）
- ✅ **結帳顯示**（小計用品項折扣後金額、折扣顯示自動折扣+手動折扣）
- ✅ **訂單明細**（訂單標題完整折扣、小計下方折扣分層、品項折扣獨立欄位）
...
- ✅ **UI/UX 優化改進**：
  - 訂單數區塊添加「>> 詳細資料」點擊提示（懸停變色）
  - 清除所有資料改名為「系統重置」（更準確表達功能）
  - 訂單明細支持折疊功能（預設收起，點擊展開/收起）
  - **訂單明細批次操作**：新增「全部展開」和「全部收起」小連結（搜尋框下方）
  - **效能優化**：移除展開/收起動畫，點擊即時響應無卡頓
  - 訂單明細按訂單編號遞減排序（最新訂單在最上方）
  - 優化觸控體驗（懸停/點擊視覺回饋，流暢動畫）

**待開發**：
- ⏳ Vue 3 專案實際建置（狀態管理架構已準備完成）
- ⏳ IndexedDB 資料庫整合（使用 Dexie.js）
- ⏳ 列印機整合（ESC/POS 協定，網路列印機）
- ⏳ 平均客單價顯示（報表統計）
- ⏳ 資料持久化（LocalStorage → IndexedDB 遷移）
- ⏳ 完整測試與效能優化
...
**檔案編碼規範**：✅ 所有檔案必須使用 UTF-8 編碼，避免亂碼產生

**觸控優化完成**：✅ 所有按鈕已使用 touchstart 事件（主畫面、規格選擇、購物車、結帳），快速連續點擊無卡頓
