...
>
> **📑 文件導覽**：[README](./README.md) | [POS-PLANNER](./POS-PLANNER.md) | [REFACTORING](./REFACTORING-SUMMARY.md) | [VUE3-MIGRATION](./VUE3-MIGRATION-GUIDE.md) | [VUE3-QUICKSTART](./VUE3-QUICKSTART.md) | [DOCS-STRUCTURE](./DOCS-STRUCTURE.md)
...
### 專案目標
開發一款專為夜市燒烤攤位設計的 iPad POS 點餐系統，簡化點餐、結帳、出單流程，提供銷售資料分析功能，提升營運效率。
...
```javascript
{
  ...
  subtotal: 260,              // 原價小計
  totalItemDiscount: 0,       // Phase 2：品項折扣總額
  discount: 0,                // 自動折扣金額（單一品項/分類/總數量折扣）
  total: 260,                 // 實收金額（subtotal - totalItemDiscount - discount）
  paymentMethod: "cash",
  receivedAmount: 1000,
  changeAmount: 740,
...
- `itemDiscount`：品項折扣金額（Phase 2 新增，點擊品項時設定）
- `totalItemDiscount`：所有品項折扣的總額
- `discount`：自動折扣金額（單一品項/分類/總數量折扣）
- 計算邏輯：小計（原價）- 品項折扣總額 - 自動折扣 = 總計
...
- [x] 優化結帳畫面折扣顯示
- [x] 小計使用品項折扣後金額
- [x] 折扣顯示自動折扣 + 手動折扣（不含品項折扣）
- [x] 修正所有總計計算邏輯
- [x] 更新以下函數：
  - `setFullAmount()` - 收全額包含所有折扣
  - `clearAmount()` - 歸零提示包含所有折扣
  - `addAmount()` - 快速金額按鈕找零包含所有折扣
  - `calculateChange()` - 手動輸入找零包含所有折扣
  - `updateCheckoutTotal()` - 已正確實作
  - `completePayment()` - 已正確實作

**實作成果**：
- ✅ 結帳介面順序：訂單項目 → **小計**（品項折扣後） → **折扣顯示**（自動折扣 + 手動折扣） → **總計** → **💵 現金付款** → 實收金額 → 找零 → 確認付款
- ✅ 小計顯示品項折扣後的金額
- ✅ 折扣顯示自動折扣 + 手動折扣
- ✅ 總計計算正確：小計 - 自動折扣 - 手動折扣
- ✅ 「收全額」按鈕自動填入應收總額
...
**狀態**：Phase 1 原型持續優化（UI/UX 改進、功能完善），準備 Vue 3 轉換

**更新記錄**：
- v5.7 (2026-02-10)：規格選擇觸控優化：① 為所有規格按鈕添加 touchstart 事件監聽器（消除 300ms 延遲）② 移除規格按鈕的 CSS 過渡動畫（transition: none）③ 添加按鈕按下視覺回饋（.pressed class，scale(0.95)）④ 添加觸覺回饋（震動 10ms）⑤ 使用 requestAnimationFrame 優化統一規格套用（延遲執行購物車更新）⑥ 區分點擊和滾動（移動 > 10px 視為滾動）⑦ 覆蓋規格選擇彈窗、飲料規格、統一規格等 40+ 個按鈕⑧ 快速連續切換口味按鈕無卡頓，達到與主畫面一致的流暢度
- v5.6 (2026-02-10)：檔案編碼規範更新：① 新增檔案編碼規範說明（所有檔案必須使用 UTF-8）② 更新文件導覽（補完整個文件連結）③ 確認 prototype.html 實際功能狀態（狀態管理架構、觸控優化、UI/UX 改進）④ 更新 CLAUDE.md 編碼規範和日期
- v5.5 (2026-02-08)：完成折扣系統與訂單明細優化：① 完善品項折扣功能（規格選擇時設定折扣）② 優化購物車顯示（��項總額右下角、顯示單價×數量、品項折扣獨立欄位）③ 結帳畫面優化（小計使用品項折扣後金額、折扣顯示自動折扣+手動折扣）④ 訂單明細優化（訂單標題顯示完整折扣、小計下方顯示自動折扣+手動折扣、品項明細獨立顯示品項折扣）⑤ 修正所有折扣計算邏輯（品項折扣、自動折扣、手動折扣分層處理）⑥ 訂單儲存結構優化（新增 totalDiscount 欄位、discount 欄位只含自動折扣+手動折扣）
- v5.4 (2026-02-08)：完成品項折扣功能優化：① 將手動折扣功能移至品項規格選擇畫面（點擊品項時可設定折扣）② 支援品項等級的折扣金額（-$10、-$20、-$50、自訂金額）③ 購物車和結帳畫面正確計算品項折扣總計（顯示分層：🏷️ 自動折扣、💰 品項折扣）④ 結帳畫面隱藏訂單等級的手動折扣區塊⑤ 修正總計計算邏輯（原價小計 - 品項折扣 - 自動折扣）
- v5.3 (2026-02-08)：優化訂單明細交互體驗：① 新增「全部展開」和「全部收起」連結功能（搜尋框下方的小連結，非大按鈕）② 移除展開/收起動畫效果提升響應速度（移除 slideDown 動畫和箭頭旋轉 transition）③ 使用 requestAnimationFrame 優化批次操作④ 新增 null 檢查避免錯誤⑤ 點擊訂單展開/收合無卡頓，即時響應
- v5.2 (2026-02-08)：完成 UI/UX 優化改進：① 訂單數區塊添加點擊提示（「>> 詳細資料」右下角提示，懸停變橙色）② 清除所有資料改名為「系統重置」（更準確表達功能，使用 🔄 圖示）③ 訂單明細支持折疊功能（預設收起，點擊標題展開/收起，箭頭旋轉動畫）④ 訂單明細按訂單編號遞減排序（最新訂單在最上方）⑤ 優化觸控體驗（懸停/點擊視覺回饋，流暢動畫）
...
- v4.8 (2026-02-07)：優化匯出結帳明細功能，支援日期區間查詢匯出；自動讀取多天資料並合併統計；區間查詢時保留匯出按鈕只隱藏關帳按鈕；修正所有繁體中文亂碼問題
...
- v4.2 (2026-02-07)：調整階段規劃，將多種付款方式從 Phase 3 移至 Phase 4；Phase 3 現在專注於庫存管理、會員系統、掃碼點餐；Phase 4 包含電子支付、AI分析、LINE機器人、財務報表
